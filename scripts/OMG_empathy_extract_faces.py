...
# Install opencv and dlib to run this script!
        ...
        if not os.path.exists(savePathActor):
            ...
            while check:
                ...
                if img is not None:
                    ...
                    if (
                        lastImageWithFaceDetected == 0
                        or lastImageWithFaceDetected > faceDetectorPrecision
                    ):
                    ...
                    try:
                        if not len(dets) == 0:
                            for i, d in enumerate(dets):
                                ...
                                cv2.imwrite(
                                    savePathActor + "/%d.png" % imageNumber, croped
                                )
                        else:
                            cv2.imwrite(
                                savePathActor + "/%d.png" % imageNumber, imageActor
                            )
                    ...
                    try:
                        if not len(dets) == 0:
                            for i, d in enumerate(dets):
                                ...
                                cv2.imwrite(
                                    savePathSubject + "/%d.png" % imageNumber, croped
                                )
                        else:
                            cv2.imwrite(
                                savePathActor + "/%d.png" % imageNumber, imageSubject
                            )
